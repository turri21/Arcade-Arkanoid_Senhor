...
check_dependencies() {
  if [[ $OSTYPE == darwin* ]]; then
    for j in flips unzip md5 cat cut; do
      command -v ${j} > /dev/null 2>&1 || exit_with_error "This script requires\n${j}"
    done
  else
    for j in flips unzip md5sum cat cut; do
      command -v ${j} > /dev/null 2>&1 || exit_with_error "This script requires\n${j}"
    done
  fi
}
...
uncompress_zips() {
  ...
  for z in "${zips[@]}"; do
    echo "Unzipping $z"
    unzip -qq -d ${BASEDIR}/${tmpdir}/ ${BASEDIR}/${z}
    if [ $? != 0 ] ; then
...
generate_rom() {
  for i in "${ifiles[@]}"; do
      ...
      if [ ! -f "${BASEDIR}/${tmpdir}/${i}" ]; then
        rm -rf ${BASEDIR}/$tmpdir
        exit_with_error "Provided ${zips[@]}\nare missing required file:\n\n${i}"
      else
...
{
  flips ${BASEDIR}/$patchfile ${BASEDIR}/${tmpdir}/${ofile}
}
